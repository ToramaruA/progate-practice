...
describe("getPostContent", () => {
  ...
  test("returns content on fetchPost success", async () => {
    const ret = await getPostContent(1);
    expect(ret).toBe("Mock Post Content");
    expect(fetchPost).toHaveBeenCalledWith(1); // fetchPostが1を引数に呼ばれたかを確認
    expect(fetchPost).toBeCalledTimes(1); // fetchPostが1回呼ばれたことを確認
  });
});
